...
def stack_rgb_geotiffs_parallel(
    input_directories: List[str], output_directory: str, max_workers: int = None
):
    """
    ...
    if free_space < 10:  # Adjust this threshold as needed
        print(
            f"Warning: Only {free_space:.2f} GB free on disk. This may not be enough."
        )
    ...
    skipped_existing = sum(
        1 for r in results if r.startswith("Skipped (already exists):")
    )
    skipped_missing = sum(
        1 for r in results if r.startswith("Skipped (missing input):")
    )
    skipped_bands = sum(
        1 for r in results if r.startswith("Skipped (insufficient bands):")
    )
...
if __name__ == "__main__":
    input_directories = [
        "data/2022/05/B04",  # Red band
        "data/2022/05/B03",  # Green band
        "data/2022/05/B02",  # Blue band
    ]
    output_directory = "data/2022/05/RGB"
